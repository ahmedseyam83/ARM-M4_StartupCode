...
void Reset_Handler(void)
{
	u32 Local_u32Datasize,Local_u32Counter,Local_u32BssSize;
	u32 *DataSrcPtr=(&(e_text)+1);
	u32 *DataDestAddress = (&s_data);
	u32 *BssDestAddress = (&s_bss);
	//reallocate .data from flash to ram
	/*Size calculations of .data section*/
	Local_u32Datasize= (u32)&e_data - (u32)&s_data ; 
	
	for(Local_u32Counter = 0 ; Local_u32Counter<Local_u32Datasize ; Local_u32Counter++)
	{
	...
	/*Size calculations of .bss section*/
	Local_u32BssSize = &(e_bss) - &(s_bss);
	for(Local_u32Counter = 0 ; Local_u32Counter<Local_u32BssSize ; Local_u32Counter++)
	{
		*BssDestAddress = 0ul;
		BssDestAddress++;
	}
